...
class Hyperparameters:
    ...
    train_seq_len: int = int(os.environ.get("TRAIN_SEQ_LEN", os.environ.get("TRAIN_MAX_SEQ_LEN", 256)))
    ...
    warmdown_iters: int = int(os.environ.get("WARMDOWN_ITERS", 500))
    max_wallclock_seconds: float = float(os.environ.get("MAX_WALLCLOCK_SECONDS", 600.0))

    # Model (defaults match the current baseline setup).
    vocab_size: int = int(os.environ.get("VOCAB_SIZE", 1024))
    num_layers: int = int(os.environ.get("NUM_LAYERS", 7))
    ...
    rope_base: float = float(os.environ.get("ROPE_BASE", 50.0))
    ...
    tied_embed_lr: float = float(os.environ.get("TIED_EMBED_LR", 0.09))
    matrix_lr: float = float(os.environ.get("MATRIX_LR", 0.08))
    scalar_lr: float = float(os.environ.get("SCALAR_LR", 0.08))
